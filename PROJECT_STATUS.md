...
- إضافة مشروع **مِتريفا (METRIVA)** كدراسة حالة مهنية موسّعة: `data/projects/metriva.json` (سبع كفاءات، بنود تحدي/حل/دور/مخرجات، كلمات مفتاحية، بطاقة معلومات سريعة، شعار المشروع)، مع توسيع مخطط `case_study` في `project.html` وحقول `admin/config.yml` بحقول اختيارية جديدة (`quick_facts`, `overview_*`, `challenge_items_*`, `solution_items_*`, `role_items_*`, `competencies_*`, `outputs_*`, `key_takeaway_*`, `tags_*`, وحقل `logo` على مستوى المشروع) — التوسيع **متوافق تماماً مع المشاريع القديمة** (يُستخدم مسار عرض قديم تلقائياً إن لم تتوفر الحقول الموسّعة؛ تم اختباره فعلياً على `teacher-promotion-guide` و`accreditation-tracking-matrix` دون أي كسر). أُضيفت بطاقة METRIVA أول عنصر في `index.html#projects` (`p7_t`/`p7_p` في `main.js`)، وشعار المشروع في `assets/images/projects/metriva-logo.jpg`. اختُبرت الصفحة محلياً عبر `python -m http.server` (لا أخطاء console)، وصُحّح خطأ أولي في قراءة حقول الكفاءات (`title`/`desc` بلا لاحقة لغة داخل قوائم `competencies_ar/en` المقسومة أصلاً حسب اللغة). دُفعت هذه التعديلات إلى فرع `cursor/metriva-case-study-page`، ثم دُمجت في `master` (دمج مباشر/fast-forward، بلا أي تعارض) بعد تأكيد المستخدم صريحاً بطلب النشر، ونُشرت فعلياً على `bandaralasmari.com` — تحقّقت من نجاح تشغيل GitHub Actions (`Deploy to Cloudflare Workers`, run 33890808648, conclusion: success) ومن استجابة الصفحة والبيانات الحيّة فعلياً عبر `curl` (`/project?slug=metriva` ← 200، `/data/projects/metriva.json` ← 200 ومحتواه صحيح، شعار المشروع ← 200، وبطاقة المشروع ظاهرة في `main.js`/الصفحة الرئيسية الحيّة)، مع تأكيد أن صفحات المشاريع القديمة وباقي الصفحات (`certificates.html`, `licenses.html`, `badges.html`, `/admin/`) لم تتأثر (نفس سلوك إعادة التوجيه 307 من `.html` إلى مسار بلا امتداد الموجود مسبقاً لكل الصفحات، وليس عطلاً جديداً).

## آخر نسخة منشورة

- آخر commit مدفوع بنجاح إلى `master`: `80bce66` — "Add METRIVA professional case study project page" (راجع `git log -1` للتأكد من الـhash الفعلي وقت القراءة إن تغيّر لاحقاً). تحقّق دائماً فعلياً عبر GitHub Actions API/الصفحة الحقيقية، لا تفترض النجاح.
